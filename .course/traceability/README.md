...
- `TRAZABILIDAD_M0.md`: vista humana interna única, generada desde el contrato.
- `generate_human_traceability.py`: genera y comprueba esa vista humana.
- `validate_m0.py`: gate mecánico de consistencia guía ↔ teoría ↔ código ↔ trazabilidad.

Cada paso registra `theory_refs`, artefactos y acción (`CREATE/MODIFY/USE/DELETE/RESTORE/VERIFY`), símbolos, comandos, observables, verificación y estado `PERMANENT`/`TEMPORARY`.
